...
#region 仅编辑器测试用，打包自动移除
#if UNITY_EDITOR
		// 标记InitMap是否被调用
		public bool Test_InitMapCalled { get; private set; }
		// 存储传入的参数，用于测试断言
		public Node[,] Test_LastInputNodes { get; private set; }
		public Vector2Int Test_LastInputSize { get; private set; }

		// 测试专用重置方法，TearDown时调用清理标记
		public void Test_ResetMapRecord()
		{
			Test_InitMapCalled = false;
			Test_LastInputNodes = null;
...
#if UNITY_EDITOR
			// 编辑器环境记录调用信息，用于测试
			Test_InitMapCalled = true;
			Test_LastInputNodes = nodes;
			Test_LastInputSize = mapSize;
#endif
		}
